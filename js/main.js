...
function checkIt(evt) {
    ...
    if (charCode > 31 && (charCode < 48 || charCode > 57)) {
        status = "EL campo solo acepta numeros."
        alert(status);
        return false
    }
    status = ""
...
function updateGoods() {
    ...
    if (Usuarios == "") {
        alert('debe seleccionar almenos un Usuario');
        return false
    }
...
if (btnregisterformlink) {
    btnregisterformlink.onclick = function () {
        mostrarR()
    };
}
if (btnloginformlink) {
    btnloginformlink.onclick = function () {
        LoginR()
    };
}
if (formregister) {
...
function ajax_post_loginUsers() {
    ...
    if (username == "") {
        alert('el campo de correo es necesario')
        return false
    }

    if (password == "") {
        alert('el campo de password es necesario')
        return false
    }
